#!/usr/bin/env bash
# Start Arize Phoenix trace viewer for lib/aor packs.
# Traces appear at http://localhost:6006 while this is running.
#
# Prerequisites:
#   1. lib/aor/.env exists with PHOENIX_TRACING=true
#   2. uv is installed (https://docs.astral.sh/uv/)
#
# Usage: ./scripts/start_aor_tracing.sh

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LIB_AOR="$REPO_ROOT/lib/aor"
